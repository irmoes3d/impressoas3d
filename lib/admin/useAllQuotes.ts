...
import type { CustomQuote, QuoteServiceType } from "@/lib/types";

interface QuoteRow {
  id: string; name: string; whatsapp: string; email: string; service_type: QuoteServiceType | null; description: string; quantity: number;
  ...
  useEffect(() => {
    ...
    supabase.from("custom_quotes").select("*, quote_files(id,name,size_kb,type)").order("created_at", { ascending: false })
      .then(({ data }) => setQuotes(((data ?? []) as QuoteRow[]).map((row) => ({
        id: row.id, serviceType: row.service_type ?? "impressao_3d", name: row.name, whatsapp: row.whatsapp, email: row.email, description: row.description,
